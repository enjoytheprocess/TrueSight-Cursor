...
AI runtime contract for this initialized federated workspace.

## Mode assumptions
- Repository mode: `federated`
- Adoption path: `sideload/adapt`
...
3. Read `components/COMPONENT_REPOS.md`, `workspace.manifest.yaml`, and the linked spec or component-repo docs.
4. Read `docs/guides/template-sideloading.md` only when adopting or upgrading the template.
5. Open optional registers only when the task uses them:
...
- Use sibling or explicit external refs in traceability when code lives outside this repo.
- This repo is the planning hub; component implementation may happen elsewhere.
